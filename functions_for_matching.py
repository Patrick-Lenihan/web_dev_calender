...
def store_info_about_matching(user,start_date,end_date,start_time,end_time):
	db = get_db()
	db.execute("""INSERT INTO matchingRequests (user,startDate,endDate,startTime,endTime,last_edit) VALUES(?,?,?,?,?,?);""",(user,start_date,end_date,start_time,end_time,'1'))
...
def generate_matched_data_times(list_of_times):
	...
	for group in list_of_times:
		...
		for time in range(0,len(group[1])-1):
			if not((group[1][time+1] - group[1][time]) == 1):
				list_of_ranges.append(start)
				list_of_ranges.append(group[1][time])
				start = group[1][time+1]
			elif time == len(group[1])-2:
				list_of_ranges.append(start)
				list_of_ranges.append(group[1][time])
